...
with open(file_to_save, "a") as txt_file:
    ...
    for county_name in county_votes:
        ...
        countyvotetotal = county_votes[county_name]
        # 6c: Calculate the percentage of votes for the county.
        countyvotetotal_percentage = float(countyvotetotal) / float(total_votes) * 100

         # 6d: Print the county results to the terminal.
        county_results=(
            f"{county_name}:{countyvotetotal_percentage:.1f}%({countyvotetotal:,})\n")

         # 6e: Save the county votes to a text file.
    
        txt_file.write(county_results)
         # 6f: Write an if statement to determine the winning county and get its vote count.
        if (countyvotetotal > largest_turnout_count):
            County_with_largest_turnout = county_name
            largest_turnout_count = countyvotetotal
